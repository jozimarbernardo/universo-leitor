...
export default async function (request) {
  ...
  const AIRTABLE_TABLE_INDICACOES = 'Indicacoes'; // Nome da tabela SEM acento
  
  // Pega os dados que o site enviou (o ID do registro e a nova contagem)
  const { recordId, newLikeCount } = await request.json();
  
  const url = `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${AIRTABLE_TABLE_INDICACOES}/${recordId}`;
